...
function crud(router, table, columns) {
  ...
  router.get('/', async (_req, res) => {
    try {
      const { rows } = await pool.query(`SELECT * FROM ${table} ORDER BY id DESC LIMIT 500`);
      res.json(rows);
    } catch (err) {
...
// Sessions (id is TEXT not SERIAL, handle separately)
const sessionsRouter = express.Router();

sessionsRouter.get('/', async (_req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM sessions ORDER BY last_seen DESC LIMIT 500');
    res.json(rows);
...
sessionsRouter.put('/:id', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'UPDATE sessions SET last_seen = NOW() WHERE id = $1 RETURNING *', [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Not found' });
    res.json(rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
// Pageviews
app.use('/api/pageviews', crud(express.Router(), 'pageviews', [
...
// Activity events
app.use('/api/activity', crud(express.Router(), 'activity_events', [
  'session_id', 'url', 'page', 'batch_ts', 'event_kind', 'event_data'
]));

// Page exits
app.use('/api/exits', crud(express.Router(), 'page_exits', [
  'session_id', 'url', 'page', 'entered_at', 'exited_at'
]));

// Errors
app.use('/api/errors', crud(express.Router(), 'errors', [
  'session_id', 'url', 'page', 'error_type', 'message',
  'source', 'line', 'col', 'stack', 'raw'
]));
