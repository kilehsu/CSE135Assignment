...
We configured enhanced access and error logging for the test site, including client hints headers (`Sec-CH-UA`, `Sec-CH-UA-Platform`, `Sec-CH-UA-Mobile`). These are requested via the `Accept-CH` response header and logged in our custom Apache log format. We also set an extended log format with: LogFormat "%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\" \"%{Sec-CH-UA}i\" \"%{Sec-CH-UA-Mobile}i\" \"%{Sec-CH-UA-Platform}i\" \"%{Sec-CH-Viewport-Width}i\" \"%{Sec-CH-Device-Memory}i\" \"%{Sec-CH-DPR}i\" \"%{Save-Data}i\"" test_extended
Also sent additional headers with Header set Accept-CH "Sec-CH-UA, Sec-CH-UA-Mobile, Sec-CH-UA-Platform, Sec-CH-Viewport-Width, Sec-CH-Device-Memory, Sec-CH-DPR, Save-Data"
