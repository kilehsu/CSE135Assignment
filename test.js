...
(function () {
  ...
  function reportError(errorData) {
    ...
    window.dispatchEvent(
      new CustomEvent("collector:error", {
  ...
  function send(payload) {
    ...
    window.dispatchEvent(
      new CustomEvent("collector:beacon", { detail: payload }),
    );
  }
  ...
  function collect(type) {
    ...
    window.dispatchEvent(
      new CustomEvent("collector:payload", { detail: payload }),
    );
  }
  ...
  const publicAPI = {
    ...
    init: function (options) {
      ...
      if (!isSampled()) {
        console.log(
          `[Collector] Session not sampled (rate: ${config.sampleRate})`,
        );
        blocked = true;
        initialized = true;
        return;
    ...
    use: function (plugin) {
      ...
      console.log(
        `[Collector] Plugin registered: ${plugin.name || "(unnamed)"}`,
      );
    },
  };
